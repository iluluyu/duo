...
- **G2 连续曲率大圆角**（2026-09-05 暂缓）：像素级验证已证明 `SetWindowRgn` 可裁切 scrcpy 视频窗口，但运行时存在未根治的缩放卡顿、区域与 SDL 重布局竞争导致的画面裁切异常，且 1-bit 硬边只能靠描边模拟 AA（观感不达标）。默认已回退 Windows 系统圆角。重启条件与阻塞清单见 [docs/window-experience.md §3.4](docs/window-experience.md)；真 AA 的可行路径需宿主自拥有合成表面（大工程，单独立项）。
...
- ⚠️ 调试战史（全是实测换来的）：① CreateCompatibleDC/SelectObject 等住在 **gdi32.dll** 不是 user32（入口点缺失 = 静默崩溃）；② WinEvent 回调里 BeginInvoke 需先建句柄 + 防 teardown 竞态 try/catch；③ csc.exe 输出是本地化 GBK，解码要 errors=replace；④ powershell.exe 命令行会弄花 CJK（PS 走 UTF-8 BOM 脚本文件，C# exe 走 UTF-16 argv 无此问题）；⑤ WSL 里 pkill -f 的 pattern 会匹配含同样文本的自身命令行（清场与启动分两条命令）；⑥ 子代理后台 bash/孤儿进程会持续干扰实验（结束后必扫残留）；⑦ **SC_SIZE 注入不可用**：拖动时鼠标捕获在 overlay 手里，目标窗口的原生 size-move 循环收不到移动消息；⑧ **分层窗口 alpha=0 的像素点击穿透**（热区条必须 alpha=1 才可命中）；⑨ **自己 SetWindowPos 会合成 WM_MOUSEMOVE** → 输入驱动的 resize 自激成 1200Hz 风暴，必须改为定时器轮询 GetCursorPos；⑩ **SDL 窗口无视 SetWindowRgn**（返回成功但无视觉效果，与 DWM 偏好无关）——窗口圆角止步 DWM 8px，想要更大圆角只能桌面采样遮罩（未做）
- ✅ mBack 下巴（2026-09-03 夜）：单圆环居中，轻触 = 返回（keyevent 4），长按 550ms = 回桌面（keyevent 3，仅物理投屏 `--home 1`；虚拟显示器无桌面，app 模式长按无动作）；实测：tap→keyevent4 ✓ 长按→hold fired 无 home ✓
- ✅ 窗口调整三件套（同夜全通）：四边+四角隐形热区（alpha=1）捕获式 resize（50ms 轮询）；顶部中央移动区（按住拖动窗口）；保比例仿最大化（AR 0.93→0.93，1501×1608→1796×1924 居中，不盖任务栏）；下巴拖拽 = 底边+底双角 resize（实测 moves=20/30 生效）
