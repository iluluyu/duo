...
- **圆角**：目标是真正的 G2 连续圆角（✅ 已验证落地），默认 48 DIP（参照 iPhone/iPad 连续曲率比例：小窗 ≈11%、大窗 ≈4% 屏宽）、可调 0–96 DIP（测试可到 160）；先验证视频窗口裁切，再接入设置。系统圆角不等于 G2。
...
### 2. P0 — 先验证大 G2 圆角的可行路径

- [x] 做最小 Windows 原型：对真实 scrcpy 视频窗口裁切（✅ 2026-09-05 像素级验证：裸窗口 + 生产路径均生效，见 [验证记录](docs/validation/window-experience.md)；**推翻 plan.md 旧结论“SetWindowRgn 无视觉效果”**——当时无像素证据）。
- [x] 验证真实透明角、动态视频、拖动（区域随矩形变化重施加）、鼠标命中（区域外点击穿透）；性能与 DPI 留给 Windows 实拍评审；对比图已存档 `docs/validation/assets/`。
- [x] 输出结论到 `docs/validation/window-experience.md`：✅ 可落地并已接入（`--corner-radius` DIP 参数，四次超椭圆 G2 多边形区域，默认 32、0 关闭）；**已知限制：1-bit 硬边无抗锯齿**，观感与默认值适宜性待 Windows 实拍。

**后续**：设置页（任务 3）暴露 corner_mode/大小；硬边锯齿若不可接受，唯一升级路径是宿主合成（大工程，需单独立项）。
...
- 2026-09-05 Task 1 实现落地：Python（argv/env/面板）+ C# overlay（热区/比例锁/收敛/尾读）全部完成，csc 现场编译通过，真实 mirror 会话冒烟验证（overlay 日志：窗口发现→修复→1.5s 内读到 `video size from log: 3392x2400`，40s 心跳稳定，ADB 钉死生效无重启循环）。
- `.venv/bin/python -m pytest -q`：**74 passed**；ruff / mypy 全绿。仍不证明 Windows 交互（拖拽手感、收敛观感、DPI）通过——见验证清单。
