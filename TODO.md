...
- **圆角**：目标是真正的 G2 连续圆角，默认建议 32 DIP、可调 0–64 DIP；先验证视频窗口裁切，再接入设置。系统圆角不等于 G2。
...
- [ ] 做最小 Windows 原型：对真实 scrcpy 视频窗口裁切，而不是只画一个圆角控件。
- [ ] 验证真实透明角、动态视频、拖动、DPI、遮挡、鼠标命中和性能；保存对比图/短录像。
- [ ] 输出结论到 `docs/validation/window-experience.md`：可落地则接入；不可行则记录限制、保留系统圆角，不标记 G2 完成。

**停止条件**：现有原生窗口链路无法可靠裁切时，不继续尝试桌面截图盖角、不默认重写播放器；提出一条最小宿主合成路线及成本，待确认后另开任务。
